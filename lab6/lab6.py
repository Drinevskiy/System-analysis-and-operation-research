...
class Bipartite:
    ...
    def solve(self):
        ...
        while True:
            path = self.find_path()
            if not path:
                print("Путь (s, t) не найден. Алгоритм завершен.")
                break
            print(f"Итерация {iteration}: {' -> '.join(path)}")
...
if __name__ == "__main__":
    ...
    matching = Bipartite(V1, V2, edges).solve()

    print(f"\nНаибольшее паросочетание (количество ребер: {len(matching)}):")
